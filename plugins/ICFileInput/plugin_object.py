...
class ICFileInput(object):
    ...
    def load_file(self):
        ...
        if self.capture.open(file_path):
            ...
            self.video_source_bridge.open(seekable=True, length=length, constfps=True, constlen=True)
    ...
    def init_plugin(self, gui_interface, video_source_bridge):
        ...
        self.tools = uic.loadUi(os.path.join(self.plugin_path, "ui", "main.ui"))
        # Add to the main window tools tab
        gui_interface.show_window(self.tools)
    ...
    def next(self):
        ...
        timestamp = float(self.pos)/max(1, self.fps)*1000
        self.pos += 1
        if ret:
            return "BGR", frame, self.pos-1, int(timestamp)
        else:
